...
@TestPropertySource(properties = {
        ...
        "app.sms.preludeApiToken=test-token",
...
public abstract class AbstractIntegrationTest {
    ...
    protected WebClient.RequestBodySpec webClientBodySpec;
    ...
    void truncateTables() {
        ...
        databaseClient.sql("TRUNCATE TABLE favourites, pending_files_uploads, car_listing_analytics, car_listings, users RESTART IDENTITY CASCADE")
                .fetch()
                .rowsUpdated()
                .block();
    }
    ...
    void setupWebClientMock() {
        ...
        webClientBodySpec = mock(WebClient.RequestBodySpec.class);
        var headersSpec = mock(WebClient.RequestHeadersSpec.class);
        responseSpec = mock(WebClient.ResponseSpec.class);

        when(webClient.post()).thenReturn(uriSpec);
        when(uriSpec.uri(anyString())).thenReturn(webClientBodySpec);
        when(webClientBodySpec.header(anyString(), anyString())).thenReturn(webClientBodySpec);
        when(webClientBodySpec.bodyValue(any())).thenReturn(headersSpec);
        when(headersSpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(Void.class)).thenReturn(Mono.empty());
        when(responseSpec.bodyToMono(PreludeVerifyResponseDTO.class))
                .thenReturn(Mono.just(new PreludeVerifyResponseDTO("success")));
    }
}
